...
def getImage(i, threshold, webcam, surface, font, refPoints):
    ...
    This function takes an image, saves it in tmp, re-imports it into PIL so it
    ...
    # pathJPG = raw_input("Please enter the path and image name of the desired"
    # "image to be procesed:")
    imgRGBA = Image.open(pathJPG)
    imgRGBA = imgRGBA.convert("RGBA")
    imgHSV = imgRGBA.convert("HSV")

    # Let's use our getColorMean() function to identify the approximate color
    # of the green screen:
    hsvData = getColorMean(imgHSV, refPoints)

    # Check the type to make sure the function worked:
    if hsvData == None:
        print "The program could not detect your green screen or your image i"\
              "s too small!"
    else:
        H, S, V = hsvData
        ...
        # It seems that we're struggling to save the file elegantly, so let's
        # just copy it from /tmp:
...
def getColorMean(PilHsvImage, refPoints):
    """
    A function that finds 4 corner-ish pixels of an image, see if they're about
    green, and then if they are, averages them to find the approximate color of
    the green screen.

    This fucntion takes a PIL HSV image (surface) and a list of points to use
    as reference:
    """
    ...
    # Create a list to store the pixel data and a numerator to properly take
    # the avg:
    pixList = []
    numerator = 0
...
def getReferencePoints(surface, webcam, font, starFont, controlPin):
    ...
    # want to start the loop, but assume it's going to be the last time they
    # run it.
    ...
    while shouldClose == False:
        ...
        # Change value of the control var because we assume the user wants to
        # close:
        shouldClose = True
...
def setText(surface, font, message, color, location=None):
    ...
    if location == None:
        ...
        return
    # They have provided a location, so we'll go ahead and place the object
    # there:
    else:
        messageRendered = font.render(message, True, color)
        messageLoc = messageRendered.get_rect()

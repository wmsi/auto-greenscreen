...
def HSVMean(PilHsvImage):
	...
	###
	# NOTE: THESE ARE THE BASELINE EXPECTATIONS OF GREEN:
	# Make sure each pixel selected was just about green:
	# H values:
	greenHlow = 80
	greenHhigh = 140
	# S
	...
	# As a techinical note, we need to convert the above H, S, and V values to 8-bit, (or 0 to 255.)
	# This is annoying, as technically H can be 1 to 360, S and V can be 0 to 100, but it's whatever...
	greenHlow = float(greenHlow / 360.0 * 255)
	greenHhigh = float(greenHhigh / 360.0 * 255)
	greenSlow = float(greenSlow * 2.55)
	greenShigh = float(greenShigh * 2.55)
	greenVlow = float(greenVlow * 2.55)
	greenVhigh = float(greenVhigh * 2.55)

	###
	print "H:", greenHlow, greenHhigh
	print "S:", greenSlow, greenShigh
	print "V:", greenVlow, greenVhigh
	...
	for pixel in xrange(len(pixList)):
		print(pixList[pixel])
		# Check if each pixel selected is satisfactorily green:
		# Keep in mind that img.getpixel() returns a tuple as (H, S, V):
		if (greenHlow <= pixList[pixel][0] <= greenHhigh):
			# I'm going to use nested if's for readability:
			if (greenSlow <= pixList[pixel][1] <= greenShigh):
				if (greenVlow <= pixList[pixel][2] <= greenVhigh):
					numerator += 1
					H += pixList[pixel][0]
					S += pixList[pixel][1]
					V += pixList[pixel][2]
